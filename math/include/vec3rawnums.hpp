...
class Vec3
{
  public:
    ...
    RealNum Norm() const noexcept { return std::sqrt(x_ * x_ + y_ * y_ + z_ * z_); }

    constexpr RealNum SquaredNorm() const noexcept { return x_ * x_ + y_ * y_ + z_ * z_; }
...
{
    RealNum norm_inverse = Real(1) / std::sqrt(x_ * x_ + y_ * y_ + z_ * z_);

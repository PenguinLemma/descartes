...
TEST_CASE("op - : Vec3 -> Vec3 ", "[Vec3]")
{
    ...
    SECTION("(0, 0, 0) is a fixed point") { CHECK(-Vec3() == Vec3()); }
}
...
TEST_CASE("op / : Vec3 x R -> Vec3", "[Vec3]")
{
    ...
    SECTION("(0, 0, 0) is a fixed point")
    {
        Vec3 zero;
        auto k =
            GENERATE(take(100, filter(CanScalarBeUsedToDivide, random(Real(-100.0), Real(100.0)))));
        CHECK(zero / k == zero);
...
{
    SECTION("(0, 0, 0) has norm 0") { CHECK(Vec3().Norm() == 0.0); }
...
TEST_CASE("Dot : Vec3 x Vec3 -> R", "[Vec3]")
{
    ...
    SECTION("Scalar factors can be extracted from scalar product")
    {
        ...
        CHECK(Dot(k * u, v) ==
              Approx(k * Dot(u, v)).epsilon(tconst::kRelativeToleranceEqualityCheck));
        CHECK(Dot(u, k * v) ==
              Approx(k * Dot(u, v)).epsilon(tconst::kRelativeToleranceEqualityCheck));
        CHECK(Dot(u * k, v) ==
              Approx(k * Dot(u, v)).epsilon(tconst::kRelativeToleranceEqualityCheck));
    ...
    SECTION("Dot product of orthogonal vectors is 0")
    {
        ...
        CHECK(Dot(v, Vec3(v.Z() - v.Y(), v.X() - v.Z(), v.Y() - v.X())) ==
...
TEST_CASE("Cross : Vec3 x Vec3 -> Vec3", "[Vec3]")
{
    ...
    SECTION("Croos product has distributive property with respect to +")
    {
        ...
        CHECK_THAT(Cross(u + v, w),
                   IsComponentWiseApprox<Vec3>(tconst::kRelativeToleranceEqualityCheck,
                                               Cross(u, w) + Cross(v, w)));
        CHECK_THAT(Cross(u, v + w),
                   IsComponentWiseApprox<Vec3>(tconst::kRelativeToleranceEqualityCheck,
                                               Cross(u, v) + Cross(u, w)));
    ...
    SECTION("Cross product satisfies Jacobi identity")
    {
        ...
        Vec3 zero;
        CHECK_THAT(Cross(u, Cross(v, w)) + Cross(v, Cross(w, u)) + Cross(w, Cross(u, v)),
